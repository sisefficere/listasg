    ...
    <div className="flex flex-col items-center justify-center gap-[50px] estrutura-padding">
      <div>
        <div className="flex flex-col w-full items-center">
          <h2 className="tipo-titulo2">Classificados em "{categoria.nome} {'>'} {subcategoria.nome}"</h2>
        </div>
      </div>
      <div>
        <div className="flex flex-col items-center gap-5 w-full ">
          {anunciantes.length != 0 ? (
            anunciantes.map((el) => (
              <CardAnunciantes key={el.id} srcImage={el.src_image} nome={el.nome_empresa} descricao={el.descricao} endereco={el.endereco} contato={el.contato} />
            ))
          ) : (
            <></>
          )}
